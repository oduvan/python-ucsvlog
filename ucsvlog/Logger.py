...
class Logger(object):
    ...
    def __init__(self,
                 ...
                 # the log level which will be used if Logger object used as function
                 # means glog() is equal to glog.log() in case default_level = 'log'
                 # stored into def_log_call property as function
                 default_level='log', 
                 
                 # list of levels which will be actually saved into the log file
                 # if it is None the all levels will be stored.
                 loglev=None, 
                 
                 # list of fields name or function which fill be put in every log row
                 # right after parent index.
                 # list of possible name can be found in ucsvlog.fields.every
                 # but you can also use your own function as string name of link
                 # this list ['stacksize','fname','filename','lineno'] will be used if None pass
                 ...
                 # related folder is using for save related file paths in call info
                 # saved into same name property but add '/' at the end if if doesn't have
                 related_folder = None,
                 
                 # True - if you want to store one block in different files.
                 # useful for logging long terms blocks, like long working cron scripts
                 ...
                 # if this value is not None then every row will have with value as last cell
                 # using in Reader object to be sure that the row is 100% valid and not broken
                 # saved into same name property
                 close_row = None, 
                 ):
    ...
    def __call__(self,a,s=0):
        return self.def_log_call(a,s+1,stack_minus=3)
    ...
    def init_log_fh(self):
        '''
        init logger filehanle in case a the real file name after rendering file template was changed
        init action_log_fh and action_log_file
        '''
        new_action_log_file = self.action_log_template.format(**self.action_log_template_params())
    ...
    def _init_cur_fh(self):
        return codecs.open(self.action_log_file,'a','utf8',buffering=self.action_log_buffering)
    ...
    def lbd_tlog(self,logname):
        return lambda data,stack=0, stack_minus=2: self.tlog(logname,data if isinstance(data,(list,tuple)) else [data],stack+2, stack_minus=stack_minus)
    
    
        '''
    def lbd_alog(self,k):
        return lambda a,b,s=0: self.alog(a,[k,a,b],s+4)
        '''
    
    def lbd_alog(self,logname):
        return lambda search,data=None,stack=0: self.alog(search, logname, data, stack+3)
    ...
    def alog(self,search,logname,data=None,stack=3):
        ...
        data = self.tlog('a_'+logname,[search] + params,stack, stack_minus=3)
        self.aindex_stack.append([data[0],search])
    
    '''
    def lbd_clog(self,k):
        return lambda a,b,s=0: self.clog(a,[k,a,b],s+4)
    '''
        
    def lbd_clog(self, logname):
        return lambda search, data=None ,stack=0: self.clog(search, logname, data, stack+3)
    ...
    def clog(self,search,logname, data, stack=3):
        ...
        for item in reversed(self.aindex_stack):
            if item[1] == search:
                ...
                self.tlog('c_'+logname, [search] + params,stack, stack_minus=3)
    ...
    def tlog(self,name,params,stack=1, stack_minus=0):
        ...
        arr_row = [self.gen_record_key(),self.get_parent_record_key()] +\
                arr_funcs_call(self.func_fields,sys._getframe(stack),self, stack=stack-stack_minus) +\
                [name] + params
